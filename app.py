...
def plot_trustworthiness_vs_k(X, X_embedded_dict, y, max_k=50):
    ...
    for method_name, X_embedded in X_embedded_dict.items():
        ...
        for k in k_values:
            trust_score = metrics_calc.calculate_trustworthiness(X, X_embedded, k=k)
...
def render_advanced_results_panel(X, X_processed, embeddings_dict, y, class_names, meta):
    ...
    with st.expander("🔎 Résultats avancés", expanded=True):
        ...
        with st.spinner("🔄 Génération automatique de tous les résultats avancés..."):
            try:
                # 1. Generate 3D visualizations (PCA, UMAP, t-SNE)
                st.markdown("## 📊 Visualisations 3D")

                # Create 3D visualizations for all methods
                methods_3d = ["PCA", "UMAP", "t-SNE"]
                cols = st.columns(len(methods_3d))

                for i, method in enumerate(methods_3d):
                    with cols[i]:
                        st.markdown(f"### {method} 3D")
                        try:
                            fig_3d = create_3d_visualization(X_processed, y, class_names, method)
                            st.plotly_chart(fig_3d, use_container_width=True)
                        except Exception as e:
                            st.error(f"Erreur {method}: {str(e)}")

                # 2. Generate Trustworthiness curves
                ...
                try:
                    ...
                    st.error(f"Erreur courbes trustworthiness: {str(e)}")

                # 3. Generate KNN Confusion matrices for all methods
                st.markdown("## 🎯 Matrices de Confusion KNN")

                k_neighbors = 5  # Default k
                method_names = list(embeddings_dict.keys())

                # Create columns for confusion matrices
                n_methods = len(method_names)
                if n_methods > 0:
                    cols_cm = st.columns(min(n_methods, 3))  # Max 3 columns

                    for i, method_name in enumerate(method_names):
                        col_idx = i % 3
                        with cols_cm[col_idx]:
                            st.markdown(f"### {method_name}")
                            try:
                                X_emb = embeddings_dict[method_name]
                                fig_cm, accuracy = create_knn_confusion_matrix(X_emb, y, class_names, k_neighbors)
                                st.pyplot(fig_cm)
                                st.metric("Précision", f"{accuracy:.2%}")
                            except Exception as e:
                                st.error(f"Erreur {method_name}: {str(e)}")
                ...
                try:
                    ...
                    with col1:
                        st.success("✅ CSV généré automatiquement!")
                        st.download_button(
                ...
                # 5. 3D Relief for Digits (if applicable)
                if meta and meta.get('name', '').lower() in ['digits', 'digits (mnist)', 'mnist']:
                    st.markdown("## 🏔️ Relief 3D (Dataset Digits)")

                    # Show relief for first few digits
                    digits_to_show = min(5, len(set(y)))  # Show max 5 different digits
                    available_digits = sorted(list(set(y)))[:digits_to_show]

                    cols_relief = st.columns(len(available_digits))

                    for i, digit in enumerate(available_digits):
                        with cols_relief[i]:
                            st.markdown(f"### Chiffre {digit}")
                            try:
                                fig_relief, error = create_digits_3d_relief(X, y, class_names, digit)
                                if fig_relief is not None:
                                    st.plotly_chart(fig_relief, use_container_width=True)
                                else:
                                    st.error(error)
                            except Exception as e:
                                st.error(f"Erreur relief chiffre {digit}: {str(e)}")

                st.success("✅ Tous les résultats avancés ont été générés automatiquement!")

            except Exception as e:
                st.error(f"❌ Erreur lors de la génération automatique: {str(e)}")
